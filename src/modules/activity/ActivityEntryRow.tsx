...
export const ActivityEntryRow = ({ event }: { event: ActivityEvent }) => {
  ...
  return (
    <div className={"flex items-start gap-6 relative max-w-[735px] pb-10"}>
      ...
      <div
        className={cn(
          "w-10 h-10 shrink-0 relative rounded-full border-0 bg-neutral-100 border-neutral-200 flex items-center justify-center uppercase text-sm font-medium text-neutral-600 dark:bg-nb-gray-900 dark:border-nb-gray-800 dark:text-nb-gray-300",
        )}
      >
      ...
      <div className={"flex flex-col w-full gap-2"}>
        <div className={"flex justify-between"}>
          <div>
            <div className={"flex items-center gap-2"}>
              <div
                className={
                  "w-4 h-4 rounded-full flex items-center justify-center uppercase text-[9px] font-medium bg-neutral-100 dark:bg-nb-gray-900"
                }
                style={{
              ...
              <span className={"text-sm text-neutral-800 dark:text-nb-gray-200"}>
                <TextWithTooltip
                  text={user?.name || user?.id || "System"}
                  maxChars={20}
                />
              </span>
              <span className={"text-sm text-neutral-500 dark:text-nb-gray-400 font-light"}>
          ...
          <span
            className={"flex gap-2 items-center text-neutral-500 dark:text-nb-gray-400 text-xs mr-1"}
          >
            <div className={"h-1 w-1 bg-neutral-300 dark:bg-nb-gray-700 rounded-full"}></div>
        ...
        <Card
          className={
            "w-full relative bg-white text-sm text-neutral-700 flex flex-col px-4 pt-3 pb-3 dark:bg-nb-gray-925 dark:text-nb-gray-300"
          }
        >
          <div className={"flex gap-4"}>
            <div
              className={
                "flex items-center text-neutral-700 dark:text-nb-gray-300 text-sm leading-[2]"
              }
            >
...
function VerticalLine() {
  return (
    <div
      className={
        "absolute left-0 top-0 translate-y-12 h-[56%] bg-neutral-200 dark:bg-nb-gray-900/50 w-[2px] ml-[20px] z-0"
      }
    ></div>
  );
}
