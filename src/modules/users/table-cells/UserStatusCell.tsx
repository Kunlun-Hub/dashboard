...
export default function UserStatusCell({ user }: Readonly<Props>) {
  ...
  const getStatusDisplay = () => {
    ...
    if (isPendingApproval) {
      return { text: t("users.status.pending"), color: "bg-netbird" };
    }
    if (status === "blocked") {
      return { text: t("users.status.blocked"), color: "bg-red-500" };
    }
    if (status === "invited") {
      return { text: t("users.status.invited"), color: "bg-yellow-400" };
    }
    if (status === "active") {
  ...
  const tooltipContent = isLocalAuthDisabled ? (
    ...
    <div className={"max-w-xs text-xs flex flex-col gap-2"}>
      <div>{t("users.invitedDescription")}</div>
    </div>
  ) : (
    <div className={"max-w-xs text-xs flex flex-col gap-2"}>
      <div>
        {t("users.pendingApprovalDescription")}
      </div>
    </div>
  );
